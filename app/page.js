...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#fff1f2] text-[#24090d]">
      <section className="mx-auto grid min-h-screen max-w-[1400px] gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[.9fr_1.1fr] lg:px-8">
        <div><p className="text-xs font-black uppercase opacity-50">Insight console</p><h1 className="mt-4 text-5xl font-black leading-tight sm:text-7xl">AI Analytics Insight Agent</h1><p className="mt-5 leading-8 opacity-65">An analytics insight agent starter for KPI summaries, anomaly notes, and executive takeaways.</p><div className="mt-8 border-l-2 border-current/15 pl-5">{steps.map((step) => <div className="mb-6" key={step}><p className="font-black">{step}</p><p className="text-sm opacity-55">Built into the starter workflow.</p></div>)}</div></div>
        <div className="rounded-2xl border bg-white border-rose-950/10 p-5"><form className="space-y-3" onSubmit={submit}>
              <textarea
              ...
              />
              <button
                className="min-h-12 w-full px-5 text-sm font-black transition disabled:cursor-not-allowed disabled:opacity-40 bg-rose-700 text-white hover:bg-rose-800"
